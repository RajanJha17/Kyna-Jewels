...
import {
  TrackingOrder as ITrackingOrder,
  OrderStatus,
  OrderItem,
  Address,
  TrackingEvent,
...
TrackingOrderSchema.index({ orderNumber: 1, userId: 1 });
...
TrackingOrderSchema.index({ userId: 1, createdAt: -1 });
...
TrackingOrderSchema.methods.updateFromSequelTracking = function (
  sequelData: any
): void {
...
TrackingOrderSchema.methods.mapSequelStatus = function (
  sequelCode: string
): OrderStatus {
...
// Static methods
TrackingOrderSchema.statics.findByOrderNumberAndEmail = function (
  orderNumber: string,
  email: string
) {
  return this.findOne({
...
TrackingOrderSchema.statics.findByDocketNumber = function (
  docketNumber: string
) {
  return this.findOne({ docketNumber });
};

TrackingOrderSchema.statics.findByCustomerEmail = function (
  email: string,
  limit: number = 10
) {
...
export interface TrackingOrderModel
  extends mongoose.Model<TrackingOrderDocument> {
  findByOrderNumberAndEmail(
    orderNumber: string,
    email: string
  ): Promise<TrackingOrderDocument | null>;
  findByDocketNumber(
    docketNumber: string
  ): Promise<TrackingOrderDocument | null>;
  findByCustomerEmail(
    email: string,
    limit?: number
  ): Promise<TrackingOrderDocument[]>;
}

// Add instance methods to the interface
export interface TrackingOrderDocument
  extends Omit<ITrackingOrder, "_id">,
    Document {
  userId?: string; // Add userId field
  paymentResponse?: any; // Add paymentResponse field
  images?: Array<{
    url: string;
    publicId?: string;
    uploadedAt?: Date;
...
export const TrackingOrder = mongoose.model<
  TrackingOrderDocument,
  TrackingOrderModel
>("TrackingOrder", TrackingOrderSchema);
