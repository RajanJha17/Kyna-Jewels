...
// Generate order number (not guaranteed unique due to DB index issues)
...
export const createDirectOrder = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const order = new OrderModel({
      user: userId,
      // orderNumber: generateOrderNumber(), // Temporarily removed to avoid duplicate key error
      items: orderItems,
      shippingAddress,
      paymentMethod,
...
export const shipOrder = async (req: AuthRequest, res: Response) => {
  try {
    ...
    try {
      ...
      const { createSequel247Service } = await import(
        "../services/Sequel247Service"
      );
      ...
      // Create tracking record
      const trackingOrder = await trackingService.createTrackingFromOrder(
        orderId,
        docketNumber
      );
