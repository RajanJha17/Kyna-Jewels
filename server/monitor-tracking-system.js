...
async function checkSystemHealth() {
  ...
  try {
    ...
    console.log(
      `🔄 Cron Job: ${health.cronJob.status} (${health.cronJob.frequency})`
    );
    console.log(
    ...
    if (health.recentActivity && health.recentActivity.length > 0) {
      ...
      health.recentActivity.forEach((order, index) => {
        console.log(
          `   ${index + 1}. ${order.orderNumber}: ${order.status} (${new Date(
            order.updatedAt
          ).toLocaleString()})`
        );
      });
    ...
    const manualUpdateResponse = await axios.post(
      `${SERVER_URL}/api/tracking/manual-update`
    );
    ...
    if (manualResult.success) {
      ...
      if (manualResult.data.updatedCount > 0) {
        console.log(
          `🎉 Successfully updated ${manualResult.data.updatedCount} orders`
        );
      }
    }
  } catch (error) {
...
async function main() {
  ...
  console.log(
    "   - Use POST /api/tracking/manual-update for admin manual trigger"
  );
